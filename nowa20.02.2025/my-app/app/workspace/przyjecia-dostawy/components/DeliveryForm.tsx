...
import { FormData, FormErrors, Product, Supplier, Recipient } from "../types";
...
export const DeliveryForm: React.FC<DeliveryFormProps> = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.product) newErrors.product = "Wybierz produkt";
    if (!formData.supplier) newErrors.supplier = "Wybierz dostawcę";
    if (!formData.weight || formData.weight <= 0) newErrors.weight = "Waga musi być większa od zera";
    if (!formData.boxCount || formData.boxCount <= 0) newErrors.boxCount = "Ilość kartonów musi być liczbą dodatnią";
  ...
  <Select 
    ...
    onValueChange={(value) => {
      ...
      setFormData({...formData, product: product || null});
    }}
  >
